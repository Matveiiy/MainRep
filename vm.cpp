    ...
    void execute_bytecode(const char* file_name) {
        ...
        while (true) {
            ...
            case INSTR_STORE32:
                ...
                fread(&byte, 1, 1, file);
                //cout << byte4 << ' ' << int(byte);
                *static_cast<int*>(memory+byte4)=reg_ea[byte];
                break;
            case INSTR_LOAD32:
                fread(&byte, 1, 1, file);
                fread(&byte4, 4, 1, file);
                reg_ea[byte]=*static_cast<int*>(memory+byte4);
                break;
            case INSTR_MOV:
                fread(&byte, 1, 1, file);
...
/*
bc.i_movq(EA3, 3);//end
    bc.i_movq(EA2, 1);//step
    bc.i_movq(EA0, -1);//begin
    int32_t beg=bc.label();
    bc.i_print_reg(EA0);
    bc.i_add(EA0, EA0, EA2);
    bc.i_neq(EA1, EA0, EA3);
    bc.i_jmp_if(EA1, beg);
    bc.close();
*/
